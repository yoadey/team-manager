...
## Purpose
Defines login through an external OpenID Connect provider as a second door alongside password login, never instead of it: the provider is advertised by `GET /auth/providers` with operator-configured branding, a login runs Authorization Code + PKCE with `state`, `nonce` and no `prompt` parameter (so a broker configured with a provider-selection scope forwards straight to its upstream IdP), and the callback validates `state` against a short-lived encrypted cookie before any token exchange so an unsolicited callback can never make the server call out. The ID token's verified email is the trust anchor -- an unverified claim is rejected outright -- and an account is resolved by external subject first, then by that verified address (linking it, and discarding any password parked on a never-verified row), then by provisioning a passwordless account with no team until somebody invites it. Success establishes an ordinary session cookie; the endpoints are reached by navigating, so every outcome including a rate-limit rejection is a redirect back to the login screen, carrying a deliberately coarse `login_error` code on failure and returning the browser to the root-relative path the login started from so an invite link still redeems.
