...
## Purpose
Defines self-service account creation with email + password: unauthenticated visitors can register, accounts start unverified until a single-use emailed token is consumed, unverified accounts cannot log in, registration and resend never reveal whether an email is already registered, and abandoned (never-verified) registrations are eventually cleaned up so the email address becomes reusable.
