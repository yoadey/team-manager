...
- [x] 7.7 Second independent review, full change, fresh context. Fixed: a password parked on a never-verified account survived the OIDC link and became usable once the link marked it verified (`ClearPassword`); erasure left `oidc_accounts` rows behind; the invite/deep-link path was dropped across the round trip (`return_to` + `oidcState.ReturnURL`); an unvalidated provider `name` claim reached `users.name` (`sanitizeOIDCName`); `OIDC_SCOPES` could be configured without `openid`; `validateOIDCIcon` accepted `/\host` and tab/CR/LF smuggling; the rate limiter answered a browser navigation with problem+json; the callback decoded the state cookie twice and ignored `error_description`; `resolve` held its mutex across provider discovery; `doLogin` could leave the login screen spinning; the provider glyph was an icon *name* rendered as literal text; and the busy-ownership regression test lost with the old `doLogin` was restored on `doPasswordLogin`
