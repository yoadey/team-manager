...
**Chosen: (a).** Legal-notice/privacy-policy text is multi-paragraph prose with headings and links — a poor fit for a single env var, and the existing runtime-override mechanism exists specifically for short scalar values (a DSN, a public key), not documents. A mounted-file scheme would work but adds a new content-loading path (fetch + parse + failure handling for a missing mount) for a document that, per the Non-Goals above, changes rarely and is set once at deploy time — the same moment `.env` is written.

Implementation refinement: rather than literal `.md` files plus a Markdown-parsing dependency (the repo has none, and "justify new runtime deps" in `CLAUDE.md` argues against adding one for this), content is a typed structured-data module, `frontend/src/features/legal/content.ts` (`LegalPageText { title, sections: { heading, paragraphs, list? }[] }`, keyed by locale and page id), rendered by `LegalSheet` with plain semantic elements (`h3`/`p`/`ul`) — no `dangerouslySetInnerHTML`, no parser, fully typed. Same operator workflow as originally planned: every `[BETREIBER: ...]` / `[OPERATOR: ...]` marker for every field `§5 DDG`/Art. 13 requires must be edited in that file before a production build ships — the same category of "must configure before going live" step as `JWT_PRIVATE_KEY` or `COOKIE_ENCRYPTION_KEY`, documented in `docs/operations.md`.
...
### 2. Legal pages are a new `legal` sheet type, not a new pseudo-route (revised during implementation)

Originally planned as new unauthenticated pseudo-routes in `urlState.ts`/`Root.tsx`. Implementation found a better fit: `components/SheetHost.tsx` already renders `state.sheet` **unconditionally** in `Root.tsx`, regardless of `state.phase` (`{phase === 'login' ? <Login/> : ...} <SheetHost /> <Toast />`). So a `legal` sheet type (`SheetState.legalPage: 'impressum' | 'datenschutz'`), opened via a new team-independent `openLegal(page)` action (alongside `closeSheet`, not the team-scoped `openProfile`/`openMore` in `useTeamActions`), renders identically before and after login through the existing modal/sheet chrome — no new routing branch, no duplicate close/back/title UI to build. `sheets/index.tsx`'s `sheetMeta` picks the title (`Impressum` vs. `Datenschutzerklärung`) from `sheet.legalPage`. This keeps `urlState.ts`'s `Route`/`ALL_ROUTES` (post-login, RBAC-relevant, `ensureRouteData`-backed) untouched, and reuses the exact same modal/bottom-sheet presentation every other secondary view in the app already uses.

Reachability: a small `LegalFooter` component (`features/legal/components/LegalFooter.tsx`) calling `openLegal` is rendered in `Login.tsx` (below the card, covering the login/password/register views alike since `Register` renders inside `Login`'s card) and in `ProfileSheet` (`features/team/components/NavSheets.tsx`, under a new "Rechtliches" section) — the latter reached from the always-visible avatar/profile button in both `MobileShell` and `DesktopShell`. A persistent on-screen footer bolted onto the authenticated `Shell` itself was considered and rejected: the shell is a fixed `100vh` flex layout (bottom nav bar on mobile, fixed sidebar on desktop) with no scrolling document-level footer region, and forcing one in would be a disproportionate chrome change for what `ProfileSheet` already reaches in one tap from anywhere in the app.
