...
type mockAuthService struct {
	...
	loginWithOIDC      func(ctx context.Context, provider string, claims auth.OIDCClaims) (string, *auth.UserRow, auth.OIDCLoginOutcome, error)
}
...
func (m *mockAuthService) LoginWithOIDC(ctx context.Context, provider string, claims auth.OIDCClaims) (string, *auth.UserRow, auth.OIDCLoginOutcome, error) {
...
func TestHandler_GetMyPhoto_Found_RedirectsToPresignedURL(t *testing.T) {
	...
	svc := &mockAuthService{
		getMyPhotoURL: func(ctx context.Context, userID string) (string, error) {
			return "https://s3.example.com/bucket/users/" + userID + "/photo?sig=abc", nil
		},
	}
	h := auth.NewHandler(svc, slog.Default(), nil, nil)
	ctx := auth.ContextWithUser(context.Background(), testUser())
