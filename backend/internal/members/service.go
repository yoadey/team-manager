...
// presignTTL bounds how long a presigned GET URL for a member's photo stays
// valid. Mirrors teams.presignTTL/auth.presignTTL.
...
type memberRepo interface {
	...
	GetMemberPhotoKey(ctx context.Context, teamID, userID string) (string, error)
}
...
// GetMemberPhotoURL returns a short-lived presigned GET URL for the profile
// photo of userID, who must be a member of teamID. Returns pgx.ErrNoRows if
// userID is not a member of teamID or has no photo set.
func (s *Service) GetMemberPhotoURL(ctx context.Context, teamID, userID string) (string, error) {
	key, err := s.repo.GetMemberPhotoKey(ctx, teamID, userID)
	if err != nil {
	...
	if err != nil {
		...
		return "", fmt.Errorf("members.Service.GetMemberPhotoURL: presign: %w", err)
	}
	return url, nil
}
