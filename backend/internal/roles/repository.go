...
// ErrLastSettingsAdmin is returned when DeleteRole would remove the last role
// assignment in the team that grants settings:write. A custom (non-system)
// role can grant settings:write just like the built-in Admin role, and
// membership_roles cascades on role deletion (ON DELETE CASCADE), so deleting
// such a role can silently strip every member holding it of admin access in
// one step — the same unrecoverable "locked out of settings" state that
// members.SetRoles/RemoveMember already guard against for individual
// membership changes.
var ErrLastSettingsAdmin = errors.New("cannot delete the last role granting settings management permission")
