...
type ServerInterface interface {
	...
	// (GET /teams/{teamId}/users/{userId}/photo)
	GetUserPhoto(w http.ResponseWriter, r *http.Request, teamId TeamId, userId UserId)
}
...
// Get another team member's profile photo (membership + members:read gated; use /auth/me/photo for the caller's own photo, which needs no module permission).
// (GET /teams/{teamId}/users/{userId}/photo)
func (_ Unimplemented) GetUserPhoto(w http.ResponseWriter, r *http.Request, teamId TeamId, userId UserId) {
	w.WriteHeader(http.StatusNotImplemented)
}
...
func (siw *ServerInterfaceWrapper) GetUserPhoto(w http.ResponseWriter, r *http.Request) {
	...
	handler := http.Handler(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		siw.Handler.GetUserPhoto(w, r, teamId, userId)
	}))
...
func HandlerWithOptions(si ServerInterface, options ChiServerOptions) http.Handler {
	...
	r.Group(func(r chi.Router) {
		r.Get(options.BaseURL+"/teams/{teamId}/users/{userId}/photo", wrapper.GetUserPhoto)
	})
...
type GetUserPhotoRequestObject struct {
	TeamId TeamId `json:"teamId"`
	UserId UserId `json:"userId"`
}

type GetUserPhotoResponseObject interface {
	VisitGetUserPhotoResponse(w http.ResponseWriter) error
}

type GetUserPhoto302Response = PhotoRedirectResponse

func (response GetUserPhoto302Response) VisitGetUserPhotoResponse(w http.ResponseWriter) error {
	w.Header().Set("Location", fmt.Sprint(response.Headers.Location))
	w.WriteHeader(302)
	return nil
}

type GetUserPhoto404ApplicationProblemPlusJSONResponse struct {
	NotFoundApplicationProblemPlusJSONResponse
}

func (response GetUserPhoto404ApplicationProblemPlusJSONResponse) VisitGetUserPhotoResponse(w http.ResponseWriter) error {
...
type StrictServerInterface interface {
	...
	// Get another team member's profile photo (membership + members:read gated; use /auth/me/photo for the caller's own photo, which needs no module permission).
	// (GET /teams/{teamId}/users/{userId}/photo)
	GetUserPhoto(ctx context.Context, request GetUserPhotoRequestObject) (GetUserPhotoResponseObject, error)
}
...
// GetUserPhoto operation middleware
func (sh *strictHandler) GetUserPhoto(w http.ResponseWriter, r *http.Request, teamId TeamId, userId UserId) {
	var request GetUserPhotoRequestObject

	request.TeamId = teamId
	request.UserId = userId

	handler := func(ctx context.Context, w http.ResponseWriter, r *http.Request, request interface{}) (interface{}, error) {
		return sh.ssi.GetUserPhoto(ctx, request.(GetUserPhotoRequestObject))
	}
	for _, middleware := range sh.middlewares {
		handler = middleware(handler, "GetUserPhoto")
	}
	...
	if err != nil {
		sh.options.ResponseErrorHandlerFunc(w, r, err)
	} else if validResponse, ok := response.(GetUserPhotoResponseObject); ok {
		if err := validResponse.VisitGetUserPhotoResponse(w); err != nil {
			sh.options.ResponseErrorHandlerFunc(w, r, err)
		}
	} else if response != nil {
		sh.options.ResponseErrorHandlerFunc(w, r, fmt.Errorf("unexpected response type: %T", response))
