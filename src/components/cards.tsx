...
export function EventCard({ e }: { e: TeamEvent }) {
  ...
  return (
    <ButtonBase
      ...
      sx={{ display: 'flex', gap: '13px', width: '100%', textAlign: 'left', background: '#fff', border: `1px solid ${NEUTRAL.line}`, borderRadius: '18px', p: '13px 15px', alignItems: 'stretch', opacity: cancelled ? 0.62 : isPast ? 0.92 : 1, justifyContent: 'flex-start' }}
    >
      <Box sx={{ flex: '0 0 50px', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', background: tm.bg, color: tm.on, borderRadius: '12px', p: '8px 0' }}>
        <Box sx={{ fontSize: '20px', fontWeight: 800, lineHeight: 1 }}>{day.getDate()}</Box>
        <Box sx={{ fontSize: '10px', fontWeight: 700, textTransform: 'uppercase' }}>{new Intl.DateTimeFormat('de-DE', { month: 'short' }).format(day).replace('.', '')}</Box>
      </Box>
      <Box sx={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', gap: '5px', justifyContent: 'center' }}>
        ...
        <Box sx={{ fontSize: '15px', fontWeight: 600, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', textDecoration: cancelled ? 'line-through' : 'none' }}>{e.title}</Box>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: '12px', fontSize: '12px', color: NEUTRAL.secondary, flexWrap: 'wrap' }}>
      ...
      <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', justifyContent: 'space-between', gap: '6px' }}>
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', gap: '7px', fontSize: '12px', fontWeight: 600 }}>
          <Box component="span" sx={{ color: '#2E7D32' }}>{e.summary.yes}✓</Box>
          <Box component="span" sx={{ color: '#BA1A1A' }}>{e.summary.no}✕</Box>
          {e.summary.maybe ? <Box component="span" sx={{ color: '#9A5B00' }}>{e.summary.maybe}?</Box> : null}
        </Box>
      </Box>
    </ButtonBase>
  );
...
export function NewsCard({ n, compact = false }: { n: NewsItem; compact?: boolean }) {
  ...
  return (
    <Box sx={{ background: '#fff', border: `1px solid ${NEUTRAL.line}`, borderRadius: '16px', p: '15px 16px' }}>
      ...
      <Box sx={{ fontSize: '13px', color: NEUTRAL.onSurfaceVariant, lineHeight: 1.5, display: '-webkit-box', WebkitLineClamp: compact ? 2 : 99, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>{n.body}</Box>
    </Box>
  );
}
