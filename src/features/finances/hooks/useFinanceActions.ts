...
export function useFinanceActions({ api, S, setState, loadFinances, loadStats, refreshMembers, askConfirm, toastMsg }: FinanceFeatureDeps) {
  ...
  const openPenaltyAssign = useCallback(() => { if (!S().members || !S().members.length) refreshMembers(); const f = S().finances; const first = (f && f.penalties[0]) ? f.penalties[0].id : null; setState({ sheet: { type: 'penaltyAssign' }, form: { userId: '', penaltyId: first } }); }, [S, refreshMembers, setState]);
  const savePenaltyAssign = useCallback(async () => { const f = S().form; if (!f.userId) { toastMsg('Bitte Person wählen'); return; } if (!f.penaltyId) { toastMsg('Bitte Strafe wählen'); return; } setState({ busy: 'save' }); await api.finances.assignPenalty(S().activeTeamId!, { userId: f.userId, penaltyId: f.penaltyId }); await loadFinances(); setState({ busy: null, sheet: null }); toastMsg('Strafe erfasst'); }, [api, S, setState, loadFinances, toastMsg]);
  ...
  const saveContrib = useCallback(async () => { const f = S().form; setState({ busy: 'save' }); await api.finances.updateContribution(f.id, { label: f.label, amount: f.amount }); await loadFinances(); setState({ busy: null, sheet: null }); toastMsg('Beitrag gespeichert'); }, [api, S, setState, loadFinances, toastMsg]);
