...
export function useFinanceActions({ api, S, setState, loadFinances, loadStats, refreshMembers, askConfirm, toastMsg }: FinanceFeatureDeps) {
  const openTxForm = useCallback((tx?: any) => {
    const f = tx ? { id: tx.id, type: tx.type, title: tx.title, amount: String(tx.amount), category: tx.category } : { type: 'income', title: '', amount: '', category: 'Beiträge' };
  ...
  const saveTx = useCallback(async () => {
    ...
    if (!title.ok) { toastMsg(title.message!); return; }
    const amount = validateMoneyAmount(f.amount, { field: 'Betrag der Buchung', positive: true });
    if (!amount.ok) { toastMsg(amount.message!); return; }
    setState({ busy: 'save' });
    if (S().sheet!.mode === 'edit') await api.finances.updateTransaction(f.id, { type: f.type, title: title.value!, amount: amount.value!, category: f.category });
    else await api.finances.addTransaction(S().activeTeamId!, { type: f.type, title: title.value!, amount: amount.value!, category: f.category });
    await loadFinances();
  ...
  const openPenaltyForm = useCallback((p?: any) => setState((st) => ({
    sheet: { type: 'penaltyForm', mode: p ? 'edit' : 'create', back: (st.sheet && st.sheet.type === 'penaltyCatalog') ? st.sheet : null },
  ...
  const savePenalty = useCallback(async () => {
    ...
    if (!label.ok) { toastMsg(label.message!); return; }
  ...
  const deletePenaltyDef = useCallback((id: string) => askConfirm({
    ...
    onConfirm: async () => { await api.finances.deletePenalty(id); await loadFinances(); setState({ sheet: { type: 'penaltyCatalog' } }); toastMsg('Strafe entfernt'); },
  ...
  const savePenaltyAssign = useCallback(async () => {
    const f = S().form;
    if (!f.userId) { toastMsg('Bitte Person wählen'); return; }
    if (!f.penaltyId) { toastMsg('Bitte Strafe wählen'); return; }
    setState({ busy: 'save' });
  ...
  }, [api, loadFinances, toastMsg]);

  const openContribForm = useCallback((c: any) => setState({
  ...
  const togglePenalty = useCallback(async (id: string) => {
    await api.finances.togglePenaltyPaid(id);
    await loadFinances();
  }, [api, loadFinances]);

  const toggleContribution = useCallback(async (id: string) => {
    await api.finances.toggleContribution(id);
    await loadFinances();
  }, [api, loadFinances]);

  const setStatsRange = useCallback((range: DateRange | null) => {
    setState({ statsRange: range, stats: null });
    loadStats(range);
  }, [setState, loadStats]);

  return { openTxForm, saveTx, deleteTx, openPenaltyCatalog, openPenaltyForm, savePenalty, deletePenaltyDef, openPenaltyAssign, savePenaltyAssign, deleteAssignment, openContribForm, saveContrib, togglePenalty, toggleContribution, setStatsRange };
}
