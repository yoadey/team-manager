...
export function usePollActions({ api, S, setState, loadPolls, toastMsg }: PollFeatureDeps) {
  ...
  const savePoll = useCallback(async () => {
    ...
    if (!poll.ok) { toastMsg(poll.message!); return; }
    setState({ busy: 'save' });
    await api.polls.create(S().activeTeamId!, { question: poll.value!.question, options: poll.value!.options, multiple: f.multiple, anonymous: f.anonymous });
    await loadPolls();
    setState({ busy: null, sheet: null });
    toastMsg('Umfrage erstellt');
  }, [api, S, setState, loadPolls, toastMsg]);

  const togglePollOption = useCallback((poll: Poll, optId: string) => {
