...
export function useAbsenceActions({ api, S, setState, refreshEvents, loadAbsences, askConfirm, toastMsg }: AbsenceDeps) {
  const openAbsenceForm = useCallback((absence?: { id: string; from: string; to: string; reason: string } | null) => {
  ...
  const removeAbsence = useCallback((id: string) => {
    askConfirm({
      ...
      onConfirm: async () => { await api.absences.remove(id); await Promise.all([refreshEvents(), loadAbsences()]); toastMsg('Abwesenheit entfernt'); },
    });
