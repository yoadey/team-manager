...
Each team member has roles; each role has per-module permission levels (`none | read | write`). Modules: `events`, `members`, `finances`, `news`, `polls`, `settings`, `stats`. Permissions are stored as JSONB in Postgres.

**Route-to-module mapping is generated from the spec, not hand-maintained.** Every team-scoped operation in `openapi.yaml` carries an `x-rbac-module` extension (`events | members | finances | news | polls | settings | stats | public`) and, where applicable, `x-rbac-self-service: true`. `cmd/genrbac` (wired into `make generate`, after oapi-codegen) parses these into `internal/middleware/rbac_table.gen.go` — DO NOT EDIT; edit the extensions in `openapi.yaml` and re-run `make generate`. `genrbac` fails the build if any team-scoped operation is missing `x-rbac-module`, so a newly added route can't silently end up unclassified. Adding a module (as `stats` did) isn't a single-point change — `genrbac`'s `validModules`, `internal/middleware/authz.go`'s `hasWritePermission`/`hasAnyPermission`, `teams.PermissionsJSON` and every place that constructs/folds it (`teams`, `roles`, `members`), and the frontend's `ModuleKey`/`MODULE_LABELS`/`ROUTE_MODULE` all hand-mirror the module list and need the new module added in lockstep.

Enforcement (`internal/middleware/authz.go`, `RequirePermission`): looks up the request's method+path in the generated table via `matchRBACRoute`. **A request whose method+path matches no table entry is rejected with 404 for every HTTP method, including GET** — there is no fallback to "unrestricted". `module: public` routes (team info itself, photo, logo, absences, notifications) require nothing beyond membership, for any method. For module-gated routes, mutating requests (POST/PUT/PATCH/DELETE) require `write`, as expected — but GET requests are *also* gated, requiring at least `read`; a module set to `none` hides read access too, not just writes. Self-service routes (an event's own attendance/comments, a poll vote, the caller's own last-selected stats range — `x-rbac-self-service: true`) never require `write` on their module regardless of method, but still require at least `read` where the route reads back module data (e.g. `polls/vote` returns the assembled poll) — self-service exempts a caller from `write`, not from `none`. `stats` gates the statistics overview/matrix/per-member views and the caller's saved date-range presets; defining a preset (create/rename/delete) requires `stats:write`, while viewing statistics and saving the last-selected range only require `stats:read` (see the `stats-access-control` capability spec under `openspec/specs/`). `notifications` has no route-level gate at all (`x-rbac-module: public`) since it aggregates across modules — instead, `notifications.Service.List` filters each returned item server-side by the caller's permission on that item's originating module.

Events carry a **controlled, narrowly-scoped exception** to the otherwise-universal `AND team_id = $N` per-request invariant: an event may optionally target additional teams beyond its owning `team_id` (`event_teams(event_id, team_id)`, one row per targeted team including the owner — see the `cross-team-events` capability spec). This is layered entirely on top of the RBAC middleware above, not inside it — `RequirePermission` still authorizes every request against the single `{teamId}` in the URL, exactly as today. What changes is only what `internal/events`' repository queries count as "this event belongs to `{teamId}`": read/RSVP paths (`GetEvent`, `ListEvents`, comments, attendance list/RSVP/nominations, event status) relax their scoping from `team_id = $N` to `EXISTS (SELECT 1 FROM event_teams …)`, so a caller reaching the event through *any* one of its targeted teams' own `{teamId}` URL is let through — a single-team event (the common case, exactly one `event_teams` row) sees identical behavior, since the EXISTS check then reduces to the old equality check. Creating or retargeting a cross-team event additionally requires `events:write` in *every* targeted team, checked by hand in `events.Service` (via the same `members.Repository.GetPermissions` the middleware itself uses) — not by the generated table, which has no concept of "more than one team per request". Deleting a cross-team event stays restricted to the owning team only, deliberately not extended to the join table. Every other module (finances, news, polls, member profiles, stats, notifications) remains strictly single-team; this exception does not generalize.
...
| `OIDC_REDIRECT_URL` | `<PUBLIC_BASE_URL>/api/v1/auth/oidc/callback` | Must match the redirect URI registered at the provider exactly. The default assumes the browser reaches this API under the frontend's own origin (the single-ingress layout the Helm chart produces) — **set it explicitly when the API is served on a separate host**, since `PUBLIC_BASE_URL` names the frontend and nothing tells the backend its own public API origin. The effective value is logged at startup. |
| `OIDC_SCOPES`     | `openid profile email`      | Space-separated base scopes for the authorization request. `openid` is always requested regardless of what this is set to — without it the provider returns no ID token and there is nothing to verify. |
